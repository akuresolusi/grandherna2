	...
	<table>
		...
		<?php
			foreach ($list_group_poli as $value) {
				...
				echo"
				<tr>
					<td colspan='12' id='poli'>Poli : <span>".$poliklinik['poliklinik']."</span></td>
				</tr>";
				$i=0;
				...
				foreach ($reg as $value2){
					...
					$ruangan = $this->detail_m->detail_ruangan($value2['idruangan']);
					echo"
					<tr valign='top'>
						<td>".$i."</td>
						<td>".$value2['noreg']."</td>
						...
						<td>".$value2['jam']."</td>
						<td>".date_format(date_create($value2['tanggalkeluar']),'d M Y')."</td>
						<td>".$value2['jamkeluar']."</td>
